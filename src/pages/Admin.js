...
export default function Admin() {
  ...
  const loadSession = async (session) => {
    ...
    const [, msgRes] = await Promise.all([
      ...
      fetch(`/api/chat?action=get-messages&sessionId=${session.sessionId}`),
    ]);
    const data = await msgRes.json();
    const msgs = data.messages || [];
    setMessages(msgs);
    if (data.visitorLastReadAt) {
      const lastAdmin = [...msgs].reverse().find((m) => m.sender === "admin");
      setVisitorRead(
        !!lastAdmin &&
          new Date(data.visitorLastReadAt) >= new Date(lastAdmin.timestamp),
      );
    } else {
    ...
    sessionsRef.current.forEach((session) => {
      ...
      channel.bind("new-message", (data) => {
        ...
        if (selectedSessionRef.current?.sessionId === session.sessionId) {
          ...
          setUnreadSessions((prev) => {
            const n = new Set(prev);
            n.add(session.sessionId);
            return n;
          });
        }
      });
      channel.bind("visitor-read-receipt", () => {
        if (selectedSessionRef.current?.sessionId === session.sessionId) {
          setVisitorRead(true);
        }
      });
  ...
  const hasNext = selectedSession
    ? sessions.findIndex((s) => s.sessionId === selectedSession.sessionId) <
      sessions.length - 1
    : false;
  ...
  return (
    <div className="admin-layout">
      ...
      <div
        className={`admin-panels${selectedSession ? " admin-panels--chat-active" : ""}`}
      >
        {/* Session list */}
              ...
              >
                <div className="admin-session-item-content">
                  {editingSession === session.sessionId ? (
                    <div
                      className="admin-session-edit"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <input
                        className="admin-session-edit-input"
                        value={editName}
                      ...
                      <div className="admin-session-edit-actions">
                        <button
                          className="admin-session-edit-save"
                          onClick={(e) => handleSaveEdit(e, session.sessionId)}
                        >
                          Save
                        </button>
                        <button
                          className="admin-session-edit-cancel"
                          onClick={(e) => {
                            e.stopPropagation();
                            setEditingSession(null);
                          }}
                    ...
                    <>
                      ...
                      <div className="admin-session-preview">
                        ...
                        <span className="admin-session-preview-text">
            ...
            <>
              ...
              <div ref={messagesContainerRef} className="admin-messages">
                {messages.map((msg, i) => (
                  <div
                    key={i}
                    className={`admin-bubble admin-bubble--${msg.sender}`}
                  >
                    {msg.sender === "visitor" && (
                ...
                {visitorRead && messages.length > 0 && messages[messages.length - 1].sender === "admin" && (
                  <span className="admin-read-receipt">Read</span>
                )}
              </div>
              <form
                onSubmit={(e) => {
